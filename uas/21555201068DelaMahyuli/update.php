...
$koneksi = mysqli_connect('localhost', 'root', '', 'B');
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($_FILES['photo']['name'] !== '') {
        ...
        $query = "UPDATE software SET tanggal_rilis='$tanggal_rilis', jenis_software='$jenis_software', harga=$harga, photo='$photo' WHERE kode='$kode'";
    } else {
        // Query untuk update data tanpa foto baru
        $query = "UPDATE software SET tanggal_rilis='$tanggal_rilis', jenis_software='$jenis_software', harga=$harga WHERE kode='$kode'";
    }
    ...
    // Jika akses bukan dari metode POST, ambil kode software dari URL parameter
    if (isset($_GET['kode'])) {
        $kode = $_GET['kode'];

        // Query untuk mengambil data software berdasarkan kode
        $query = "SELECT * FROM software WHERE kode='$kode'";
